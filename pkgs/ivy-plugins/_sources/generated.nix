# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  bqf = {
    pname = "bqf";
    version = "17680cda3538913e88dd4c6456c837db9ace40ae";
    src = fetchFromGitHub {
      owner = "kevinhwang91";
      repo = "nvim-bqf";
      rev = "17680cda3538913e88dd4c6456c837db9ace40ae";
      fetchSubmodules = false;
      sha256 = "sha256-ls7hlXRSO+Dhd8l+qRAMUF/MkuMuiYXkV6hyCyAKxcc=";
    };
    date = "2025-08-03";
  };
  catppuccin = {
    pname = "catppuccin";
    version = "30fa4d122d9b22ad8b2e0ab1b533c8c26c4dde86";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "nvim";
      rev = "30fa4d122d9b22ad8b2e0ab1b533c8c26c4dde86";
      fetchSubmodules = false;
      sha256 = "sha256-o8VLMPriOh4+Ay5Ff0cWQYXjmihdr3x9131bKHHTsQE=";
    };
    as = "catppuccin";
    date = "2025-08-19";
  };
  chai-nvim = {
    pname = "chai-nvim";
    version = "5b3142493114ade681dfad63417e5989c11f0f6d";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "chai.nvim";
      rev = "5b3142493114ade681dfad63417e5989c11f0f6d";
      fetchSubmodules = false;
      sha256 = "sha256-T2KHSYdRNbqAPpZrzzRBkTmhS8YV1eiHfnRR1QG5D9k=";
    };
    as = "chai";
    start = "true";
    date = "2025-03-21";
  };
  cloak = {
    pname = "cloak";
    version = "648aca6d33ec011dc3166e7af3b38820d01a71e4";
    src = fetchFromGitHub {
      owner = "laytan";
      repo = "cloak.nvim";
      rev = "648aca6d33ec011dc3166e7af3b38820d01a71e4";
      fetchSubmodules = false;
      sha256 = "sha256-V7oNIu7IBAHqSrgCNoePNUPjQDU9cFThFf7XGIth0sk=";
    };
    date = "2024-06-12";
  };
  crates = {
    pname = "crates";
    version = "ac9fa498a9edb96dc3056724ff69d5f40b898453";
    src = fetchFromGitHub {
      owner = "saecki";
      repo = "crates.nvim";
      rev = "ac9fa498a9edb96dc3056724ff69d5f40b898453";
      fetchSubmodules = false;
      sha256 = "sha256-jfmST/S9ymwgQ99PTCOlJkk5zaxE5HiDV16TmTISDII=";
    };
    date = "2025-08-23";
  };
  direnv = {
    pname = "direnv";
    version = "4dfc8758a1deab45e37b7f3661e0fd3759d85788";
    src = fetchFromGitHub {
      owner = "NotAShelf";
      repo = "direnv.nvim";
      rev = "4dfc8758a1deab45e37b7f3661e0fd3759d85788";
      fetchSubmodules = false;
      sha256 = "sha256-KqO8uDbVy4sVVZ6mHikuO+SWCzWr97ZuFRC8npOPJIE=";
    };
    date = "2025-06-09";
  };
  evergarden = {
    pname = "evergarden";
    version = "29b388dc6f803649b49098c83e0aba4e7839e20d";
    src = fetchFromGitHub {
      owner = "everviolet";
      repo = "nvim";
      rev = "29b388dc6f803649b49098c83e0aba4e7839e20d";
      fetchSubmodules = false;
      sha256 = "sha256-Mc4f9lLG44vvul7OfelmcxVqlxnrcz+uGGZHJnAfT0c=";
    };
    as = "evergarden";
    date = "2025-09-14";
  };
  fidget = {
    pname = "fidget";
    version = "2f7c08f45639a64a5c0abcf67321d52c3f499ae6";
    src = fetchFromGitHub {
      owner = "j-hui";
      repo = "fidget.nvim";
      rev = "2f7c08f45639a64a5c0abcf67321d52c3f499ae6";
      fetchSubmodules = false;
      sha256 = "sha256-8Gl2Ck4YJGReSEq1Xeh1dpdRq4eImmrxvIHrfxdem3Q=";
    };
    date = "2023-11-09";
  };
  freeze = {
    pname = "freeze";
    version = "590fdb0d724485f578e4fee0e3ae2b32040dfb1a";
    src = fetchFromGitHub {
      owner = "charm-community";
      repo = "freeze.nvim";
      rev = "590fdb0d724485f578e4fee0e3ae2b32040dfb1a";
      fetchSubmodules = false;
      sha256 = "sha256-VdFmSRBP7Up0j5+rp7ZkfTdQmduJjsl6EZ9gf27vgZg=";
    };
    date = "2025-03-25";
  };
  fzf-lua = {
    pname = "fzf-lua";
    version = "c2d9b8693792c6dd810162d9e8c90782cec19404";
    src = fetchFromGitHub {
      owner = "ibhagwan";
      repo = "fzf-lua";
      rev = "c2d9b8693792c6dd810162d9e8c90782cec19404";
      fetchSubmodules = false;
      sha256 = "sha256-saREanyWzAhnZZxL7ByXp0fWbRxElwJTgnRKqcDcKuU=";
    };
    date = "2025-09-09";
  };
  hunk-nvim = {
    pname = "hunk-nvim";
    version = "1e0a4d719c780bb8b0690a54915601508ced321e";
    src = fetchFromGitHub {
      owner = "julienvincent";
      repo = "hunk.nvim";
      rev = "1e0a4d719c780bb8b0690a54915601508ced321e";
      fetchSubmodules = false;
      sha256 = "sha256-va7cCBuBk3WLAZjKl+lxlKmVckaK010RAdWMzrXlzAw=";
    };
    date = "2025-06-09";
  };
  keymaps-nvim = {
    pname = "keymaps-nvim";
    version = "f606f66b2aafbd0dbb4328aeab2b89e5dd9d3473";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "keymaps.nvim";
      rev = "f606f66b2aafbd0dbb4328aeab2b89e5dd9d3473";
      fetchSubmodules = false;
      sha256 = "sha256-pqbZRu9dUh/0zhC+JTYFfb5vyq+XXxygHPbfNwDeUYk=";
    };
    date = "2025-07-31";
  };
  lylla-nvim = {
    pname = "lylla-nvim";
    version = "0e0a110a2ce1add94a1055319aa301712e2d206a";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "lylla.nvim";
      rev = "0e0a110a2ce1add94a1055319aa301712e2d206a";
      fetchSubmodules = false;
      sha256 = "sha256-JONQpuYUcD4Pn79b3fDWnl8YeKVAhhLzSYekvWJhdMU=";
    };
    date = "2025-09-03";
  };
  lz-n = {
    pname = "lz-n";
    version = "590d1fcae1c34bf7f366de87908aa990a2b555af";
    src = fetchFromGitHub {
      owner = "nvim-neorocks";
      repo = "lz.n";
      rev = "590d1fcae1c34bf7f366de87908aa990a2b555af";
      fetchSubmodules = false;
      sha256 = "sha256-e0Gf4s3+yZnPW1gSy1R4OrGXKaNWrQH7A+dPKbbsdRc=";
    };
    start = "true";
    date = "2025-09-05";
  };
  marks-nvim = {
    pname = "marks-nvim";
    version = "f353e8c08c50f39e99a9ed474172df7eddd89b72";
    src = fetchFromGitHub {
      owner = "chentoast";
      repo = "marks.nvim";
      rev = "f353e8c08c50f39e99a9ed474172df7eddd89b72";
      fetchSubmodules = false;
      sha256 = "sha256-ho2b2Ulh+GTqY0QvW7zjFOSlF5g/kaxWyOjKWhTFq7c=";
    };
    date = "2025-05-13";
  };
  mini-ai = {
    pname = "mini-ai";
    version = "45a26d032d8703f280c69c2ed9fb4e1bfc8f24f9";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.ai";
      rev = "45a26d032d8703f280c69c2ed9fb4e1bfc8f24f9";
      fetchSubmodules = false;
      sha256 = "sha256-k/5MzIUd+O1+Ru6jRhOQR9SSQEXIzXYojSxweBbkxWA=";
    };
    date = "2025-08-28";
  };
  mini-align = {
    pname = "mini-align";
    version = "ab6777ca0fdfe8e4cfe931e3b0b5d98c719c3c1d";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.align";
      rev = "ab6777ca0fdfe8e4cfe931e3b0b5d98c719c3c1d";
      fetchSubmodules = false;
      sha256 = "sha256-uulN9/E8ycuEOFZ1dCwGzsprTsQ8UEFNp2CQmaX7fUg=";
    };
    date = "2025-08-28";
  };
  mini-bracketed = {
    pname = "mini-bracketed";
    version = "672b4e2fe9222fd4d5a104ff23217801bae37046";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.bracketed";
      rev = "672b4e2fe9222fd4d5a104ff23217801bae37046";
      fetchSubmodules = false;
      sha256 = "sha256-MMXaVfR6DmLr+mNZ4u25VYthnhxt8pJaJUFYj8yGWKc=";
    };
    date = "2025-08-28";
  };
  mini-diff = {
    pname = "mini-diff";
    version = "7077b636d642fcd5cd48554e187a251883239660";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.diff";
      rev = "7077b636d642fcd5cd48554e187a251883239660";
      fetchSubmodules = false;
      sha256 = "sha256-6LRR8w/OhDiHbjcIERR8Gb6O3Ropetc8d2Db2i5/Qc4=";
    };
    date = "2025-08-28";
  };
  mini-files = {
    pname = "mini-files";
    version = "34a730a6bdb09c3e6755fe34b20a78a5d81d21c4";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.files";
      rev = "34a730a6bdb09c3e6755fe34b20a78a5d81d21c4";
      fetchSubmodules = false;
      sha256 = "sha256-Q14CtXrqYzweSUsh+d9fHMhdCEF1P+IiVoFQPt79beQ=";
    };
    date = "2025-08-28";
  };
  mini-git = {
    pname = "mini-git";
    version = "1948d4290105d4c52b8c1d5da5b748c402f6fdff";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini-git";
      rev = "1948d4290105d4c52b8c1d5da5b748c402f6fdff";
      fetchSubmodules = false;
      sha256 = "sha256-TFeeBEDEVlKeUm9j2tcgB+cUtqs6N2h7K30QyI3O8pM=";
    };
    date = "2025-08-28";
  };
  mini-icons = {
    pname = "mini-icons";
    version = "f9a177c11daa7829389b7b6eaaec8b8a5c47052d";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.icons";
      rev = "f9a177c11daa7829389b7b6eaaec8b8a5c47052d";
      fetchSubmodules = false;
      sha256 = "sha256-Br5LHQtHJ21NWBwb9TofQhQ8HaUlF7lkKBx7fpjcHwk=";
    };
    date = "2025-08-28";
  };
  mini-jump2d = {
    pname = "mini-jump2d";
    version = "3710fcdfbbff539e245e16ff14f2d95ffc2fb631";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.jump2d";
      rev = "3710fcdfbbff539e245e16ff14f2d95ffc2fb631";
      fetchSubmodules = false;
      sha256 = "sha256-4W5mvy61KVe/T+fkkQynOMVxh/TIdjGa9Rr+UiOGrUs=";
    };
    date = "2025-08-28";
  };
  mini-move = {
    pname = "mini-move";
    version = "710ff226e31a3b9dd7e14bc9e2993f24d653df20";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.move";
      rev = "710ff226e31a3b9dd7e14bc9e2993f24d653df20";
      fetchSubmodules = false;
      sha256 = "sha256-SodDSn2RIKHc2LQ+b5FwQSdqtBQ3G0gd8jSDeKtmZTU=";
    };
    date = "2025-08-28";
  };
  mini-operators = {
    pname = "mini-operators";
    version = "57c9daf96e0278ebac07bc57fe2f60cf360e5eea";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.operators";
      rev = "57c9daf96e0278ebac07bc57fe2f60cf360e5eea";
      fetchSubmodules = false;
      sha256 = "sha256-21t6porgYNBYe0bty6j7XE2nFyuEt+cNaYPv/R2oG3U=";
    };
    date = "2025-08-28";
  };
  mini-pairs = {
    pname = "mini-pairs";
    version = "3738ea30ff33e0cbf2983dc67319a5468d25b0a9";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.pairs";
      rev = "3738ea30ff33e0cbf2983dc67319a5468d25b0a9";
      fetchSubmodules = false;
      sha256 = "sha256-Mk/UbNO7QcHw2S6HLtXlyAYo4PcXwOd5e6Nq+aZi6tA=";
    };
    date = "2025-09-08";
  };
  mini-splitjoin = {
    pname = "mini-splitjoin";
    version = "93b14b5e8e8107b77724b09763dcd6de58626af7";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.splitjoin";
      rev = "93b14b5e8e8107b77724b09763dcd6de58626af7";
      fetchSubmodules = false;
      sha256 = "sha256-ycGJqJM2evAapyCohkWTiaK/4Kq2DCtEmQxMyg5XlhA=";
    };
    date = "2025-08-28";
  };
  mini-surround = {
    pname = "mini-surround";
    version = "e170a2dde4b0a8905c9bb218ed428be94ec0b255";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.surround";
      rev = "e170a2dde4b0a8905c9bb218ed428be94ec0b255";
      fetchSubmodules = false;
      sha256 = "sha256-8Sd+pq9CkbgZAvutRDiRnRUP7wfLspBLMQB03iCfoqc=";
    };
    date = "2025-09-09";
  };
  mini-trailspace = {
    pname = "mini-trailspace";
    version = "eb0756101087e941b22447c7564b79948a13f4d1";
    src = fetchFromGitHub {
      owner = "echasnovski";
      repo = "mini.trailspace";
      rev = "eb0756101087e941b22447c7564b79948a13f4d1";
      fetchSubmodules = false;
      sha256 = "sha256-a4743N4HvXw+yisp/A/b6riTsmbAO3oveCdpOT9mO4o=";
    };
    date = "2025-08-28";
  };
  mossy-nvim = {
    pname = "mossy-nvim";
    version = "1ba0b70e16bd42c95ab6dcaaf51035edc25301af";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "mossy.nvim";
      rev = "1ba0b70e16bd42c95ab6dcaaf51035edc25301af";
      fetchSubmodules = false;
      sha256 = "sha256-ZpemSwV/pH7cv3vC3Kv8Y6EpEiRawDQZovCWJKDiPFY=";
    };
    date = "2025-09-08";
  };
  neo-tree = {
    pname = "neo-tree";
    version = "ed057048a281b418d5318dd5153f9486daa517a3";
    src = fetchFromGitHub {
      owner = "nvim-neo-tree";
      repo = "neo-tree.nvim";
      rev = "ed057048a281b418d5318dd5153f9486daa517a3";
      fetchSubmodules = false;
      sha256 = "sha256-SacuWtkoaTW8ubxbfrylTjZ4a0KGEgyxWHk7iswzBpI=";
    };
    date = "2025-09-01";
  };
  neogen = {
    pname = "neogen";
    version = "d7f9461727751fb07f82011051338a9aba07581d";
    src = fetchFromGitHub {
      owner = "danymat";
      repo = "neogen";
      rev = "d7f9461727751fb07f82011051338a9aba07581d";
      fetchSubmodules = false;
      sha256 = "sha256-uXV8YZLO44lFys1RfIqsvonC2YEYwb1iiaiLfsON3hE=";
    };
    date = "2025-05-03";
  };
  nivvie-nvim = {
    pname = "nivvie-nvim";
    version = "f2ee7cd6aae80f7b3a7ed9c6f7c3b21110fb5706";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "nivvie.nvim";
      rev = "f2ee7cd6aae80f7b3a7ed9c6f7c3b21110fb5706";
      fetchSubmodules = false;
      sha256 = "sha256-sUHC146fALmpBcyk6yW8uIdBXVzu5oBHuTr/sGMq3sc=";
    };
    date = "2025-08-21";
  };
  nui = {
    pname = "nui";
    version = "de740991c12411b663994b2860f1a4fd0937c130";
    src = fetchFromGitHub {
      owner = "MunifTanjim";
      repo = "nui.nvim";
      rev = "de740991c12411b663994b2860f1a4fd0937c130";
      fetchSubmodules = false;
      sha256 = "sha256-41slmnvt1z7sCxvpiVuFmQ9g7eCaxQi1dDCL3AxSL1A=";
    };
    date = "2025-06-08";
  };
  nvim-lint = {
    pname = "nvim-lint";
    version = "f126af5345c7472e9a0cdbe1d1a29209be72c4c4";
    src = fetchFromGitHub {
      owner = "mfussenegger";
      repo = "nvim-lint";
      rev = "f126af5345c7472e9a0cdbe1d1a29209be72c4c4";
      fetchSubmodules = false;
      sha256 = "sha256-hNMvXw+IwWtLnrmu6waKgMGcaWX46EkHNgBYvLZ+XeE=";
    };
    date = "2025-08-28";
  };
  nvim-lspconfig = {
    pname = "nvim-lspconfig";
    version = "c8b90ae5cbe21d547b342b05c9266dcb8ca0de8f";
    src = fetchFromGitHub {
      owner = "neovim";
      repo = "nvim-lspconfig";
      rev = "c8b90ae5cbe21d547b342b05c9266dcb8ca0de8f";
      fetchSubmodules = false;
      sha256 = "sha256-ADnSGO1R+RBGkKzu5S/DGdTIe8Fu84nEUHiLIQuq08I=";
    };
    start = "true";
    date = "2025-09-07";
  };
  nvim-nio = {
    pname = "nvim-nio";
    version = "21f5324bfac14e22ba26553caf69ec76ae8a7662";
    src = fetchFromGitHub {
      owner = "nvim-neotest";
      repo = "nvim-nio";
      rev = "21f5324bfac14e22ba26553caf69ec76ae8a7662";
      fetchSubmodules = false;
      sha256 = "sha256-eDbzJAGdUBhTwuD0Nt9FihZj1MmVdQfn/GKIybuu5a8=";
    };
    date = "2025-01-20";
  };
  nvim-treesitter = {
    pname = "nvim-treesitter";
    version = "fee71c102c54146f90b14c89711ddbb8f95a1484";
    src = fetchFromGitHub {
      owner = "nvim-treesitter";
      repo = "nvim-treesitter";
      rev = "fee71c102c54146f90b14c89711ddbb8f95a1484";
      fetchSubmodules = false;
      sha256 = "sha256-cRKw+PIykqHcdk131sG5QKFoPheSvbIuacE3UWS0k7Q=";
    };
    start = "true";
    date = "2025-09-07";
  };
  nvim-treesitter-context = {
    pname = "nvim-treesitter-context";
    version = "66a9b5fa9e806918b5fe3dba00c6cce7e230abd2";
    src = fetchFromGitHub {
      owner = "nvim-treesitter";
      repo = "nvim-treesitter-context";
      rev = "66a9b5fa9e806918b5fe3dba00c6cce7e230abd2";
      fetchSubmodules = false;
      sha256 = "sha256-jvoksYJs+U7XX3aIKewQOjIoTZ852uRmqwpXmtSJQa4=";
    };
    date = "2025-09-03";
  };
  plenary = {
    pname = "plenary";
    version = "b9fd5226c2f76c951fc8ed5923d85e4de065e509";
    src = fetchFromGitHub {
      owner = "nvim-lua";
      repo = "plenary.nvim";
      rev = "b9fd5226c2f76c951fc8ed5923d85e4de065e509";
      fetchSubmodules = false;
      sha256 = "sha256-9Un7ekhBxcnmFE1xjCCFTZ7eqIbmXvQexpnhduAg4M0=";
    };
    start = "true";
    date = "2025-07-26";
  };
  quill-nvim = {
    pname = "quill-nvim";
    version = "c3e40509b6f4a428091fb1e42c3e992ba0e09fb0";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "quill.nvim";
      rev = "c3e40509b6f4a428091fb1e42c3e992ba0e09fb0";
      fetchSubmodules = false;
      sha256 = "sha256-Q5/2V+KvZSJ375/4lGC5x3CwcS0oSs1gB59PRTCXkn4=";
    };
    date = "2025-08-05";
  };
  rainbow-delimiters = {
    pname = "rainbow-delimiters";
    version = "687ef75fdbd497eabc9eea92b52e7b4d403b3319";
    src = fetchFromGitHub {
      owner = "hiphish";
      repo = "rainbow-delimiters.nvim";
      rev = "687ef75fdbd497eabc9eea92b52e7b4d403b3319";
      fetchSubmodules = false;
      sha256 = "sha256-4zpjuu0yvA86bBANww3LAlXUczsSyQ4MGamjBWd3FZM=";
    };
    date = "2025-09-07";
  };
  sayama-nvim = {
    pname = "sayama-nvim";
    version = "39175a766dfc80324d3130d27c3e7922f826226e";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "sayama.nvim";
      rev = "39175a766dfc80324d3130d27c3e7922f826226e";
      fetchSubmodules = false;
      sha256 = "sha256-Rxjg6RvcN5JOP5CTv7Jj7AQJv36zRMaHiFEwhiBVgfw=";
    };
    date = "2024-05-30";
  };
  schemastore = {
    pname = "schemastore";
    version = "37acbfa077c6075be976b7ac37e6be592ae56111";
    src = fetchFromGitHub {
      owner = "b0o";
      repo = "schemastore.nvim";
      rev = "37acbfa077c6075be976b7ac37e6be592ae56111";
      fetchSubmodules = false;
      sha256 = "sha256-WOfszybRmLzCUe7KUSzaaiPyVjgob3WUuoNiVGW6S+I=";
    };
    date = "2025-09-09";
  };
  shelf-nvim = {
    pname = "shelf-nvim";
    version = "686c8d2cce891ab5ffd881b88f549592a8ef40a9";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "shelf.nvim";
      rev = "686c8d2cce891ab5ffd881b88f549592a8ef40a9";
      fetchSubmodules = false;
      sha256 = "sha256-qnzVu0VwenQWGfJBhFcjWyT5B/U3vsLq+2O4VxAeA0M=";
    };
    date = "2025-08-07";
  };
  symbol-usage = {
    pname = "symbol-usage";
    version = "e07c07dfe7504295a369281e95a24e1afa14b243";
    src = fetchFromGitHub {
      owner = "Wansmer";
      repo = "symbol-usage.nvim";
      rev = "e07c07dfe7504295a369281e95a24e1afa14b243";
      fetchSubmodules = false;
      sha256 = "sha256-zWT6ZGYGpWLwuUrMlmyTIE5UZtPLX2FnywhycTxUaRQ=";
    };
    date = "2025-05-03";
  };
  tabby-nvim = {
    pname = "tabby-nvim";
    version = "b3affa6db7eab80fca2a2db5b73b473144507039";
    src = fetchFromGitHub {
      owner = "nanozuki";
      repo = "tabby.nvim";
      rev = "b3affa6db7eab80fca2a2db5b73b473144507039";
      fetchSubmodules = false;
      sha256 = "sha256-26ysSn0klZYMPEaxCe/1zD2qCYwobU5dZSq5P/GtwMU=";
    };
    date = "2025-05-23";
  };
  todo-comments = {
    pname = "todo-comments";
    version = "304a8d204ee787d2544d8bc23cd38d2f929e7cc5";
    src = fetchFromGitHub {
      owner = "folke";
      repo = "todo-comments.nvim";
      rev = "304a8d204ee787d2544d8bc23cd38d2f929e7cc5";
      fetchSubmodules = false;
      sha256 = "sha256-at9OSBtQqyiDdxKdNn2x6z4k8xrDD90sACKEK7uKNUM=";
    };
    date = "2025-01-14";
  };
  toggleterm = {
    pname = "toggleterm";
    version = "9a88eae817ef395952e08650b3283726786fb5fb";
    src = fetchFromGitHub {
      owner = "akinsho";
      repo = "toggleterm.nvim";
      rev = "9a88eae817ef395952e08650b3283726786fb5fb";
      fetchSubmodules = false;
      sha256 = "sha256-fytbX+L12TK45YKFU9K+iFJcDrwboKabihc2LtX29J4=";
    };
    date = "2025-03-09";
  };
  treewalker = {
    pname = "treewalker";
    version = "4b268cdd9d9b4bc6b06760dcf883734de84bfcd3";
    src = fetchFromGitHub {
      owner = "aaronik";
      repo = "treewalker.nvim";
      rev = "4b268cdd9d9b4bc6b06760dcf883734de84bfcd3";
      fetchSubmodules = false;
      sha256 = "sha256-kk07WIDKLd1UD4Xz/18QndJSNtJk6Xh909pucRT4/18=";
    };
    date = "2025-08-27";
  };
  trouble = {
    pname = "trouble";
    version = "85bedb7eb7fa331a2ccbecb9202d8abba64d37b3";
    src = fetchFromGitHub {
      owner = "folke";
      repo = "trouble.nvim";
      rev = "85bedb7eb7fa331a2ccbecb9202d8abba64d37b3";
      fetchSubmodules = false;
      sha256 = "sha256-au9wp88a0CutEf2f8Bi0vFTUR0zvQKgFX1vMVg4myGI=";
    };
    date = "2025-02-12";
  };
  undotree = {
    pname = "undotree";
    version = "fe9a9d0645f0f5532360b5e5f5c550d7bb4f1869";
    src = fetchFromGitHub {
      owner = "mbbill";
      repo = "undotree";
      rev = "fe9a9d0645f0f5532360b5e5f5c550d7bb4f1869";
      fetchSubmodules = false;
      sha256 = "sha256-tudR+46nd63jY1VTCNEfZ2CofxCODXaHos0+NdFI6wU=";
    };
    date = "2025-08-30";
  };
  vim-fugitive = {
    pname = "vim-fugitive";
    version = "61b51c09b7c9ce04e821f6cf76ea4f6f903e3cf4";
    src = fetchFromGitHub {
      owner = "tpope";
      repo = "vim-fugitive";
      rev = "61b51c09b7c9ce04e821f6cf76ea4f6f903e3cf4";
      fetchSubmodules = false;
      sha256 = "sha256-u39oObHCXk8uew5HyVdV1Z69Viilv3B7x1SUJxYXYLo=";
    };
    date = "2025-07-15";
  };
  vim-startuptime = {
    pname = "vim-startuptime";
    version = "b6f0d93f6b8cf6eee0b4c94450198ba2d6a05ff6";
    src = fetchFromGitHub {
      owner = "dstein64";
      repo = "vim-startuptime";
      rev = "b6f0d93f6b8cf6eee0b4c94450198ba2d6a05ff6";
      fetchSubmodules = false;
      sha256 = "sha256-0YLDkU1y89O5z7tgxaH5USQpJDfTuN0fsPJOAp6pa5Y=";
    };
    date = "2025-02-17";
  };
  yosu-nvim = {
    pname = "yosu-nvim";
    version = "b92a8d9e294025f1d249a24f0b2538c9a8a14007";
    src = fetchFromGitHub {
      owner = "comfysage";
      repo = "yosu.nvim";
      rev = "b92a8d9e294025f1d249a24f0b2538c9a8a14007";
      fetchSubmodules = false;
      sha256 = "sha256-ao7fnihe/aKyL3XAgqgIj5aVUhVS20JsYV56+QfNoJ4=";
    };
    date = "2025-05-09";
  };
  zk-nvim = {
    pname = "zk-nvim";
    version = "f079de79b4eb489861a41430ca30d705cb4041fe";
    src = fetchFromGitHub {
      owner = "mickael-menu";
      repo = "zk-nvim";
      rev = "f079de79b4eb489861a41430ca30d705cb4041fe";
      fetchSubmodules = false;
      sha256 = "sha256-eNrDRzH2QHrug2J0rkVJd/TWfIfXu2teE3OAqSje6ss=";
    };
    date = "2025-09-05";
  };
}
